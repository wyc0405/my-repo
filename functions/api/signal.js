// ============================================================
// 200슨피단 전략 시그널 API  (Cloudflare Pages Function)
//   GET /api/signal
//
...
const RANGE = "10y";         // 시뮬레이션에 쓰는 과거 데이터 기간
...
const etDate = (sec) => ET_FORMAT.format(new Date(sec * 1000)); // "YYYY-MM-DD"
...
function makePayload(dates, spx, tqqq, meta, P = PARAMS) {
  ...
  return {
    ...
    chart,
    events: sim.events.slice(-20).reverse(),
  };
}
...
export async function onRequestGet(context) {
  ...
  const cacheKey = new Request(new URL("/__cache/signal", context.request.url).toString());
